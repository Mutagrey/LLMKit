...
private struct ExampleModelsTab: View {
    ...
    private var modelSections: [ExampleModelSection] {
        let recommendedModels = catalogModels
        let sections = Array(Set(recommendedModels.map(\.family))).sorted(by: { $0.title < $1.title })
        return sections.map { section in
            let models = recommendedModels.filter {
                $0.family == section
            }
            return ExampleModelSection(
                id: section,
                models: models
            )
        }
