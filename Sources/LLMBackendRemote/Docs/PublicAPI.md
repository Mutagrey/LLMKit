...
`RemoteAPIStyle` selects the request mapping strategy. The default remains generic completions/chat mapping; OpenAI uses Chat Completions request and response shapes.
...
OpenAI-compatible response fields for `usage` and `finish_reason` are mapped into `UsageMetrics` and `StreamFinishReason` when present.
