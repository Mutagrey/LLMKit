...
Keep demo composition inside `Examples/LLMKitDemo/LLMKitDemo` instead of publishing a separate `LLMExampleUI` package target. The demo app may import concrete backend adapters and reusable UI modules so app-level wiring stays outside `LLMUIChat` and `LLMUIDownloads`.
