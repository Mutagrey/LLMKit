...
`ChatScreen` renders a backend-agnostic transcript and compact composer using `ChatViewModel`, keeps the input docked with `safeAreaInset`, uses inline navigation titles on platforms with navigation bars, and exposes system keyboard dismissal affordances without owning model selection.
...
In-flight generation state is rendered in the transcript below the latest bubble, keeping the composer focused on text input and send affordances.
...
`ChatViewModel.cancelStreaming()` cancels the in-flight task so host UI can expose an explicit stop button outside the default compact composer when needed.
